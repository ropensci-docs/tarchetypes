...
Create a target that cancels itself if a user-defined decision rule is
met.
...
- skip:
  ...
  R code for the skipping condition. If it evaluates to `TRUE` during
  [`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.html),
  the target will cancel itself.
...
- tidy_eval:
  ...
  [`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.html)).
  Applies to arguments `command` and `skip`.
...
A target definition object with `targets::tar_cancel(your_condition)`
inserted into the command. See the "Target definition objects" section
for background.

## Details

`tar_skip()` creates a target that cancels itself whenever a custom
condition is met. The mechanism of cancellation is
`targets::tar_cancel(your_condition)`, which allows skipping to happen
even if the target does not exist yet. This behavior differs from
`tar_cue(mode = "never")`, which still runs if the target does not
exist.
