...
`names` arguments, whereas `tar_hook_outer_raw()` expects evaluated
expression objects.
...
- hook:
  ...
  special placeholder symbol `.x` so `tar_hook_outer()` knows where to
  insert the original command of the target.
...
symbol `.x` so `tar_hook_outer()` knows where to insert the original
