...
frames. Not a user-side function. Do not invoke directly.

`tar_map2()` expects unevaluated language for arguments `name`,
`command1`, `command2`, `columns1`, and `columns2`. `tar_map2_raw()`
expects a character string for `name` and an evaluated expression object
for each of `command1`, `command2`, `columns1`, and `columns2`.
...
- name:
  ...
  Base name of the targets. In regular `tarchetypes` functions, the
  `name` argument is an unevaluated symbol. In the `"_raw"` versions of
  functions, `name` is a character string.
