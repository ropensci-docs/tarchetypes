...
Superseded target factories for targets with specialized storage
formats. For example, `tar_qs(name = data, command = get_data())` is
shorthand for
`tar_target(name = data, command = get_data(), format = "qs")`.

`tar_format_feather()` is superseded in favor of
...
introduction of the `aws` argument into
