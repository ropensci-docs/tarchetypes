...
Create a target that always runs if a user-defined condition rule is
met.
...
- force:
  ...
  R code for the condition that forces a build. If it evaluates to
  `TRUE`, then your work will run during
...
- tidy_eval:
  ...
  [`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.html)).
  Applies to arguments `command` and `force`.
...
condition is met, and another to respond to it and do your actual work.
See the "Target definition objects" section for background.

## Details

`tar_force()` creates a target that always runs when a custom condition
is met. The implementation builds on top of
[`tar_change()`](https://docs.ropensci.org/tarchetypes/reference/tar_change.md).
Thus, a pair of targets is created: an upstream auxiliary target to
indicate the custom condition and a downstream target that responds to
it and does your work.

`tar_force()` does not actually use
[`tar_cue_force()`](https://docs.ropensci.org/tarchetypes/reference/tar_cue_force.md),
and the mechanism is totally different. Because the upstream target
always runs,
[`tar_outdated()`](https://docs.ropensci.org/targets/reference/tar_outdated.html)
and
[`tar_visnetwork()`](https://docs.ropensci.org/targets/reference/tar_visnetwork.html)
will always show both targets as outdated. However,
[`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.html)
will still skip the downstream one if the upstream custom condition is
not met.
