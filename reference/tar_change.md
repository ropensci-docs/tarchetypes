...
Create a target that responds to a change in an arbitrary value. If the
value changes, the target reruns.
...
- tidy_eval:
  ...
  Applies to arguments `command` and `change`.
...
downstream. The upstream one triggers the change, and the downstream one
responds to it. See the "Target definition objects" section for
background.

## Details

`tar_change()` creates a pair of targets, one upstream and one
downstream. The upstream target always runs and returns an auxiliary
value. This auxiliary value gets referenced in the downstream target,
which causes the downstream target to rerun if the auxiliary value
changes. The behavior is cancelled if `cue` is `tar_cue(depend = FALSE)`
...
[`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.html)
will still skip the downstream one if the upstream target did not detect
a change.
