...
frames, where the user sets the (maximum) number of batches.

`tar_map2_count()` expects unevaluated language for arguments `name`,
`command1`, `command2`, `columns1`, and `columns2`.
`tar_map2_count_raw()` expects a character string for `name` and an
...
- batches:
  ...
  Positive integer of length 1, maximum number of batches (dynamic
  branches within static branches) of the downstream (`command2`)
  targets. Batches are formed from row groups of the `command1` target
  output.
