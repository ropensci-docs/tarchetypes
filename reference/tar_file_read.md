...
Create a pair of targets: one to track a file with `format = "file"`,
and another to read the file.
...
- command:
  ...
  R code that runs in the `format = "file"` target and returns the file
  to be tracked.

- read:

  R code to read the file. Must include `!!.x` where the file path goes:
  for example,
...
A list of two new target definition objects to track a file and read the
contents. See the "Target definition objects" section for background.
