...
With `tarchetypes`, we can simplify the pipeline with the
...
and it enforces the dependency relationships it finds. In our case, it
reads `report.Rmd` and then forces `report` to depend on `dataset`. That
way,
[`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.html)
always processes `dataset` before `report`, and it automatically reruns
